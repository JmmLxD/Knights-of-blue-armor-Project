...
public partial class BattleManager : MonoBehaviour
{
    ...
    public bool playerTurn; //current turn chooser  true -> player , false -> pc
...
public partial class BattleManager : MonoBehaviour
{
    ...
    public void LauchTurn()
    {
        if(playerTurn)
        {
            PlayerTurn turn = Instantiate(PlayerTurnManager,Vector2.zero,Quaternion.identity).GetComponent<PlayerTurn>();
            turn.battle = this;
        }
        else
        {
            EnemyTurn turn = Instantiate(EnemyTurnManager,Vector2.zero,Quaternion.identity).GetComponent<EnemyTurn>();
            turn.battle = this;
        }
        
        playerTurn = !playerTurn;
        // this is just the idea the conditions for turns and the way they change must be added later
    }
